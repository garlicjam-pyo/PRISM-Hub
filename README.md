...
최종 갱신: 2026-09-22 | 상태: **독립 검토 후 수정·재검증 진행 중. MATLAB R2024b 실행 및 PPRC Simulink 평균 모델 검증 완료. 전체 Simscape 스위칭 모델·SALS·모드 통합은 미완료.**

> **최신 실행 결과:** [MATLAB 재검증 진행 기록](docs/09_MATLAB_재검증_진행기록.md), [실행 가능한 평균 모델과 수치 결과](validation/2026-09-22/README.md). 역방향 ZCS 검출과 PPRC 전력수지를 수정했고 약80 A의 실제 SPS 전류 한계를 반영했습니다. 아래 2026-09-21 설계표와 PASS 기록 전체를 수정 후 검증 완료로 해석하면 안 됩니다.
...
> 프로젝트의 배경, 주요 약어, 각 변환단의 역할, 운전 모드와 검토 결과를 순서대로 설명한 문서는 [프로젝트 기술 개요 및 검토 결과 보고서](docs/05_프로젝트_기술개요_및_검토결과.md)입니다.

---

## 0. 독립 검토 반영 (2026-09-21)

독립 검토가 PPRC 전력수지 누락, 충전 방향 ZCS 결함, 충전 공유 버스와 고정이득 CLLC의 충돌, DAB 정격 불일치, SALS 통신 지연 반례, 15 ms LP의 무효(greedy와 동일), 선행기술(Ni TPEL 2020, Li WiPDA 2018, Hassanpour TPEL 2023, JLR US20190023203A1, Bosch EP1053129A1 등)을 지적했고 대부분 타당하다. v0.9.0에서 전력수지·역방향 ZCS·SALS 창·entrypoint를 수정했고, 아키텍처를 **충전기 포트 = Stage A 출력 노드(바이패스 폐지), PPRC = DAB(100 V 링크) + 4상한 H-브리지 직렬 포트, 최소 배터리 640 V, CLLC 5 kW**로 바꾸는 v1.2를 진행한다. 아래 절의 수치 중 검토로 바뀐 것은 대응 문서를 우선한다.
...
## 3. 기존 구조 대비 이점 (2026-09-21 계산값, 재평가 필요)
...
정직한 한계: 회로 원리(고정비 STC + 부분전력 조정)는 선행기술이 있고(Google APEC 2018, NDSU APEC 2019, TPEL 2025), 공진 SC 충전 조정 특허(GM US 10363828)와 컨버터+APM 결합 특허(EP 3987646)도 있다. 신규성은 **무조정 2:1 경로 + 충전 모드 바이패스 + 저전압 절연 직렬 셀 + 설계 규칙 4건**, 그리고 SALS 강건 스케줄링으로 한정한다.

## 4. 기존 검증 기록 (2026-09-21, 최신 판정은 위 재검증 기록 참조)
...
2. IEEE Xplore·Google Patents·Espacenet 전수 검색 — GM US 10363828, EP 3987646의 청구항 정독 후 논문 1 서론 차별화 문장 확정
3. 초고 다듬기: 논문 1 아키텍처 그림(신규 작도), 표 I–III 삽입, 논문 2 V-E 민감도 실행 후 표 작성 → 투고
