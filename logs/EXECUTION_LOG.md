# EXECUTION LOG — PRISM-Hub

각 항목: **실행 내용 → 결과 → 발견/결정**. 시뮬레이션은 Python(검증용 원본)으로 실행했고 동일 계산을 MATLAB 스크립트로 이식했다. 절대 경로는 저장소 상대 경로로 바꾸었다.

---

## 2026-09-05 ~ 09-11 — 제안서 작성 (docs/01)

- 웹 조사 6회(EV DC-DC 토폴로지, OBC/APM 통합, PPP, GaN FCML, 48 V 조널, NN 예측 제어). 400 V 충전 인프라 지배(유럽 800 V 충전기 ≈ 1 %), CLLC+재구성 토폴로지 유망, TAB 다중포트 전류 스트레스 한계, 온보드 허브에 PPP 적용 사례 미확인 → PRISM-Hub 3단 구조 제안.
- 09-11: droop 대응 방향 논의 → "부하 변동에 따른 버스 전압 droop 예측" 채택, 3.6절 ARL 신설, WP4.5·S6·KPI 6–8·리스크 추가, 일정 18주.

## 2026-09-20 — 엔지니어링 설계·검증서 (docs/02)

### 세션 1: 수치 설계 (python/calc.py → docs/fig/fig1–7)
- 참고문헌 검색 9회: Aghabali TTE 2021, Wouters TPEL 2024, Anzola IEEE Access 2020, Zientarski JESTPE, Seeman–Sanders TPEL 2008, Zhao DAB TPEL 2014, Zahid TTE 2015, Jung TPEL 2013, Xie TPEL 2025, Zhao/Blaabjerg TPEL 2021, Vazquez IEM 2014, Wood IEEE PE Mag 2025. **선행기술 발견**: He/Jiang/Nan APEC 2018(STC+PPP, 48 V), Ni et al. APEC 2019(GaN STC+PPVR, EV).
- Stage A f_s 스윕 50–400 kHz → 150 kHz 선정(캡 7.8 µF, L_r 144 nH, 99.69 %). 돌입 3.4 kA(프리차지 필수).
- Stage B ΔV 스윕 648–920 V → +77/−53 V, 최대 4.8 kW → 정격 8 kW. 충전 모드 바이패스 결정(직렬 손실 323 W는 작으나 변압기 3.75배 사이징 회피).
- Stage C: 25:3, Q 0.4, m 6 → L_r 11 µH, C_r 36.8 nF, L_m 66 µH, 이득 ±1 %. 12 V 3권선 불가 → 벅.
- 버스: CPL 안정성 37 nF(트리비얼), droop 유도식 ΔV = ΔI·t_resp/(C_ser + C_bus), f_res 14.5 kHz → (당시 가정) BW 4.8 kHz, t_resp 72 µs, 60 A → 8.7 V.
- Baseline 비교: 충전 손실 4.5 → 0.5 kW, 자기소자 180 → 12.4 kVA, 주행 절감 56 Wh/100 km.
- 자체 검토(9장): 오류 7건 정정(션트 모델 droop 39 V 오류, PPRC 25 % 전제, 3권선, 위상 셰딩 누락, Coss 50 % 계상, 홀드업 기준 오류, 1/√C 스케일).

### 세션 2: TB-A1 (python/tb_a1_sim.py → docs/fig/tba1_*)
- 단상 셀 구분선형 ODE, RK4, 주기당 680 스텝, 300주기.
- 공칭 788 V/46.9 A/f_s = f_r: I_pk 73.7 A, 스윙 20.0 V, 손실 59.1 W, η 99.68 % — 3장 계산 일치. **턴오프 전류 9.2 %** → ZCS 불성립.
- 원인: 데드타임 3 % + 출력캡 공진 이동 1 %. 미세 스윕 0.93–1.05 → 최적 0.975–0.98(2–3.5 %), 창 ±1 %. 공차 ±3–5 % 시 13–24 %, 하드 턴오프 상한 +22–43 W.
- 수치 아티팩트 2건 수정: (a) 출력캡 20 µF(C_fly의 2.6배)로 공진 18 % 이동 → 400 µF; (b) 데드타임 다이오드 영교차 채터링 → 영교차 클램프, 주기 정수 스텝.
- 결정 A-1: f_s = 0.98 f_r = 147 kHz. A-2: 적응형 ZCS 제어 필수. 파라미터 스크립트에 f_r/f_s 분리.
- 산출: matlab/tb_a1_rsc_phase.m, matlab/build_rsc_phase.m.

### 세션 3: WP3-1 / TB-A2 / A3 / A4 (python/tb_a_array.py → docs/fig/wp31_*, tba2_*, tba3_*)
- 8위상 벡터화 모델. 실행 시간 제한으로 단계별 실행(mc / a2 / a3 / a4), 주기당 340 스텝, 50–60주기.
- V1: N=1 재현 — 75.3 A / 4.4 % / 59.6 W (부록 A 일치).
- 몬테카를로 20회(C ±5 %, L ±10 %, R ±10 %): 고정 0.98 → 17/20 실패(최대 38.6 %); ZC 턴오프+공통 주기 0.90 f_r → 0/20, 손실 +10 %; **자기발진(위상별 ZC 주기 추종) → 0/20, 59.4 W**. 결정 B-1.
- 자기발진 로직 결함 발견: 무부하 근처 오검출 토글 → 에너지 역류(프리차지 12.5 ms 후 버스 붕괴). 블랭킹 창 0.80–1.12×공칭으로 해결.
- TB-A2: 모듈 편차 ≤ 1.9 %, 위상 ±10.5 %, 리플 0.21 V, 손실 471 W(V2 PASS), R_out 2.3 mΩ.
- TB-A3: 저항 직결 시 L/R 7 ns 강성으로 발산 → C_in 100 µF 추가(결정 B-2). 피크 15.6 A, t95 13.6 ms. 프리차지 없이 8 kA.
- TB-A4: 1모듈 31.6 W(V3 PASS) vs 8위상 114 W.
- 산출: matlab/tb_a_array.m.

### 세션 4: TB-B2 / TB-S1 (python/tb_b_pprc.py, tb_b_pprc2.py → docs/fig/tbs1_*)
- 평균값 직렬 경로 모델(테브냉 394 V/10.2 mΩ, L_path 1.5 µH, C_ser 100 µF, DAB 지연 10+5 µs, 샘플 5 µs, 포화 ±100 A, CPL 5 kW + 60 A).
- PI 단독: 주파수응답(해석식과 상태공간 대조 일치)에서 최대 f_c 1.8 kHz(공진 Q ≈ 10). 시간 영역 droop 8.6 V, 회복 430 µs.
  - 시뮬 버그 2건 수정: 지령 초기값 이중 계상, pre-boost 조건식 우선순위(v_ref 800 V) — 수정 전 결과 폐기.
- 상태궤환+적분(극배치, scipy place_poles): 부호 규약·플랜트 Vbat/2 누락 수정 후 f_bw 8–10 kHz 안정, droop 4.7 V, 회복 80 µs. L_path 0.5/1.5/3 µH 모두 10 kHz. 결정 C-1.
- 제어 비교: B1 4.5 V, B2 2.1 V, ARL(20 %, +50 µs) 4.7 V, pre-boost 0 V(+8 V 사전 편차).
- 스텝 스윕: 25/60/80/87/95/105 A → 2.0/4.7/6.3/14.6/붕괴/붕괴. **포화 절벽 87.5 A**, pre-boost 무력.
- 캡 축소: s=0.25 불안정(포화·공진 리밋 사이클), s=0.5 B0 7.6 V, s=2 2.7 V.
- 결론: 3.5절 가정(BW 4.8 kHz, ARL 잔여 1.7 V, 캡 1/4) 철회 → ARL 기여 = 포화 예측 회피 + 제약 인지 MPC. 부록 C, 본문 2.4/3.5/KPI 6/9장 갱신.
- 산출: matlab/tb_b_pprc.m.

### 세션 5: 이름·저장소
- 프로젝트명 PRISM-Hub 유지 결정, 제어 계층 ARL → SALS(Saturation-Aware Anticipatory Load Shaping) 개명 검토.
- 저장소 초기 구성(이 로그, README, CHANGELOG).

### 세션 6 (2026-09-21): TB-S6 / TB-C1 (python/tb_s6_sals.py, tb_c1_cllc.py)
- V4: s=1 Kaw=0 60 A → 4.7 V (부록 C 재현).
- anti-windup 스케일 버그 수정(∂u/∂q = −K5로 정규화). s=0.25: I_sat=1000 A에서도 선형 폐루프 진동(4 kHz: droop 137 V) → 원인 DAB 지연 + 29 kHz 공진. MPC 불가 판정. s=0.5 7.6 V.
- SALS 시나리오 수정 2회(초기 피크 60 A·55 A로 정격 미달 → 서지 50 A 상수 20 ms 모델로 110 A). 결과: B0 붕괴 / HW 150 A 3.9 V / SALS 완전 8.0 V / SALS 단순+오차(−20 %, ±5 ms) 붕괴 / SALS 강건(±5 ms 최악, +25 %) 4.0 V / 반응형 부하 덤프(388 V 트립) 350 V 딥 2.1 ms. 발견: 타이밍 오차 민감 → 구간 예측·최악값 제약 필수.
- TB-C1: 6상태 CLLC ODE. 이득 0.998, FHA 오차 ≤ 0.5 %(V5), Im 5.9 A(예측 6.1), rms 9.1/68.3 A, η 97.85 %, ±2 % 입력 → 47.1–49.1 V.
- 산출: matlab/tb_s6_sals.m, tb_c1_cllc.m; 부록 D·E; README 0.4.0.

### 세션 7 (2026-09-21): TB-S2 / TB-B3 / TB-C2 / WP4.5
- TB-S2: SoC 78 %에서 6 s 내 CV 미도달 → 95.5 %로 조정. 단자 331–460 V, CV 3.04 s, 경로 99.77 %.
- TB-B3: 바이패스 폐로 점프 0.09 V, 전류 피크 13.2 A, 재개방 1.2 V.
- TB-C2: 위상천이 제어 시도 → −20°에서 100 kW 순환(공진점 임피던스 ≈ 0) → φ = 0 동기 운전으로 재정의; OCV 스텝으로 방향 전환 57.5 µs; f_s 1.1 f_r 전류 제한 확인.
- WP4.5: 서스펜션 2차 모델 발산(wn·dt = 1.26) → 1차 5 ms로 교체. GBM 6모델 학습(첫 실행 300 s 초과 → 축소). 중앙값 MAE ≈ 지속 예측, q0.9 최대 과소예측 36–47 A, 폐루프 붕괴. 최악값 창 규칙(명령 후 20–220 ms 서지 가정): 지연 20/100/190 ms 전부 vmin 396 V.
- 결론: 학습 요소 조건부 축소, SALS = 규칙 기반 강건 스케줄링. 부록 F·G·H, README 0.5.0.

### 세션 8 (2026-09-21): 전면 재검토 (v1.1)
- 검토 범위: 요구사양(1.3) → 3장 수식 → 부록 A–H 결과 → 4·5장 가이드 → 스크립트 파라미터.
- 발견·정정: (12) 3.3.2 "105 A"는 직렬 정격 100 A 무시 → min(100 A, 8 kW/ΔV); TB-S5에서 600 V 코너 PPRC 8.36 kW > 8 kW, ΔV 101 V > 90 V → V_bat < 640 V 20 kW 디레이팅. (13) 4.3.1/4.3.3/4.3.8이 v1.0 제어(고정 타이밍·PI 캐스케이드·GRU/ff/MPC) 잔존 → 자기발진 ZC·상태궤환·SALS LP로 교체, C_in 추가. (14) 4.3.4 CLLC 위상천이 제어 서술 → 동기 운전 + f_s 편이(부록 G).
- TB-S3/S4/S5 실행(tb_s3_s5.py): 2.0 V / 0.6 V / 396–401 V.
- 본문 2.4·3.5·3.6·5·6.1·8·9 재작성, 제안서 정오표, prism_design_params.m SALS 구조체. README 0.6.0.

### 세션 9 (2026-09-21): 남은 항목 정리
- TB-A1x 링잉: 첫 모델(클램프 없는 RLC, 노드 0→V_dc 충전) 886 V — 물리 오류. 정정: 잔류전류의 L_r 에너지로는 노드가 460 V에 도달하지 못함(V_reach ≤ 386 V), 오버슈트는 상보 턴온 에지 여기 → t_rise·L_pkg 스윕(부록 J).
- SALS 몬테카를로 300회(축약 판정): 초과 사건 25 %, SALS 0 붕괴, 지연 중앙값 1 ms/최대 199 ms, 반응형 딥 22–58 V.
- 웹 검색 2회: [21] 교체 후보(CSITE 2021, ATE 2025), 최근접 선행연구(IEEE conf 2020 보조부하 관리, US 9371005).
- docs/03 논문 구조안, docs/04 대조표. README 0.7.0.

### 세션 10 (2026-09-21): 논문 초고·선행기술
- 원격 확인: main = 959efdf(v0.7.0). v0.7.1 패치는 사용자 적용 대기.
- 웹 검색 1회: GM 공진 SC 충전 조정 특허(US 10363828/10632863), 결합 컨버터/APM 특허(EP 3987646), IEEE Access 2024 APM, Results in Engineering 2026 PPC+ML. → 신규성 문장 축소(2.5절), [25]–[28].
- docs/06 논문 1 초고(초록·I–VII·그림 매핑), docs/07 논문 2 초고(초록·절 구성·결과 매핑). README 0.8.0.

### 세션 11 (2026-09-21): 독립 검토 대응
- 원격 e9128a9(독립 검토 17파일) 확인, 로컬 v0.7.1/v0.8.0 리베이스.
- 수정·재현: tb_b_pprc3.py(수지 보정: 648 V 78.2 A/6.02 kW, droop 5.34/4.76/4.43 V; 헤드룸 절벽 648 V 기본 20 kW +40 A 붕괴), tb_a_array.py 극성 무관 ZC(역방향 0.8–1.1 %), sals_montecarlo.py 창 기준(반례 87.5 A, MC 위반 0/300, 미공급 A·ms 통계).
- 내 정상상태 해석의 첫 버전은 DAB 전력을 이중 계상해 103 A가 나왔다 → 경로/버스 수지를 분리해 재유도(78.2 A). 검토의 이상 모델 77.4 A와 일치.
- 결정: 충전기 포트 = V_A 노드(바이패스 폐지), PPRC 2단(DAB 링크 + H-브리지), V_bat,min 640 V, CLLC 5 kW, SALS = 최악값 창 + 우선순위 승인/제한(LP·지평선 주장 철회).
- README/CHANGELOG/9.1 정정 15–24. v1.2 작업 목록은 response_v0.9.md D절.

---

## 미결 항목
- 설계검증서 v1.2(검토 대응 D절 1–7), Simscape 실행, 특허 청구항 정독, 논문 초고 재작성.
- 설계검증서 본문 2.4·3.5·4.3.3 재작성(현재는 "검증 후 갱신" 주석으로 표시).
- 참고문헌 [21](열관리 부하) 1차 문헌 교체, 선행기술 전수 검색.
- Simulink 모델 실제 생성·배선 확인(build_rsc_phase.m는 미실행).

## 세션 12 (2026-09-22): 첨부 이력 통합·유한 링크 모델

- bundle 검증 및 네 패치 커밋 존재 확인. attached/main 3c09e43와 MATLAB ddb3bb3를8ea0888에서 병합.
- Python/MATLAB6상태2단 PPRC:12사례·정상점24개·에너지 수지1,000점·시간 간격 절반 검증. 최대 언어 간 지표 차이1.91e-10.
- 기존 MATLAB/Python 핵심 회귀시험도 통합 후 통과. 이전 Simulink 모델은 이상 직렬 포트 검증용으로 보존.
- 실제 경로 전류106.922 A 반례 및84 A 지령 후보 개발. SALS 관측 전122.161 A·10 ms 반례와25 kW 예산 불일치 발견.
- NIO 특허를GM으로 표기한 오류 정정. 논문2편에서 미입증 효율·신규성·전압강건성 주장을 제거.
- 상세 결과와 미완료 항목은 docs/10_검토대응_통합개발.md가 현재 기준. 위 세션의 GM 표기와 미결 상태는 당시 기록이다.

## 세션 13 (2026-09-22): 공통 제어기·60 A 검증

- 1e7b9fb 검토의 운전점별 이득과 50/60 A 문제에 대응. 4.8 kHz 설계점의 이득을 고정한 후 80사례를 실행.
- 정상 41/41, 한 변수 공차 28/28 통과. 29 kW 과부하 3건 중 640 V 실패, 지연 10/15 µs의 4건 실패. 지연/공차 전 범위 보장은 미완료.
- 초기 5 kHz 후보의 공차 잔류 진동 실패를 보존하고 4.8 kHz로 보완. 기존 4 kHz 공통 이득의 101.52 A 반례도 검증.
- MATLAB 독립 적분에서 80개 조건의 판정 일치 및 기존 12사례의 회귀검사 통과. 코드·JSON·문서·그림은 새 검증 폴더에 기록.
- 다음 검증은 실제 지연/공차 조합, 스위칭 리플·보호·기동, 배터리/충전 모드 통합이다.
